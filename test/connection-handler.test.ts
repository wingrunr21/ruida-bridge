...
} from "../src/connection-handler.ts";
...
describe("ConnectionHandler", () => {
  ...
  describe("UDP Communication", () => {
    test("should validate UDP packet size limits", () => {
      ...
      expect(chunks.every((chunk) => chunk.length <= MAX_PAYLOAD_SIZE)).toBe(
        true,
      );
    });
  ...
  describe("Acknowledgment Handling", () => {
    test("should recognize valid ACK responses", () => {
      ...
      const isValidAck = (data: Buffer): boolean => {
        return (
          data.length === 1 &&
          (data[0] === ACK_SUCCESS || data[0] === ACK_ERROR)
        );
      };
  ...
  describe("Error Handling", () => {
    test("should handle connection errors gracefully", () => {
      ...
      expect(mockStatus.error).toHaveBeenCalledWith(
        "Connection handling error: Error: Test connection error",
      );
    });
    ...
    test("should log socket errors", () => {
      ...
      expect(mockStatus.error).toHaveBeenCalledWith(
        "Client socket error: Socket error",
      );
    });
  });
