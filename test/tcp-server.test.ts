...
describe('TcpServer', () => {
    ...
    describe('Server Lifecycle', () => {
        test('should start and emit started event', (done) => {
            server.on('started', () => {
                expect(mockStatus.ok).toHaveBeenCalledWith('TCP server listening on 127.0.0.1:15007');
                done();
            });
    ...
    describe('Connection Statistics', () => {
        ...
        test('should track connection state changes', () => {
            // Initial state
            let stats = server.getConnectionStats();
            expect(stats.current).toBe(0);
            expect(stats.processing).toBe(false);
    ...
    describe('Error Handling', () => {
        test('should handle server socket errors', () => {
            ...
            expect(mockStatus.error).toHaveBeenCalledWith('Server socket error: Server socket error');
        });
        ...
        test('should handle connection processing errors', () => {
            ...
            expect(mockStatus.error).toHaveBeenCalledWith('Failed to handle connection: Error: Connection processing failed');
        });
    });
