...
describe('Ruida Bridge Integration Tests', () => {
    ...
    describe('Protocol Message Handling', () => {
        ...
        test('should validate common Ruida commands', () => {
            ...
            const validateLaserCommand = (cmd: Buffer): boolean => {
                ...
                switch (cmd[0]) {
                    case 0xC6: // Power commands
                        return cmd.length === 4 && (cmd[1] === 0x01 || cmd[1] === 0x02 || cmd[1] === 0x21 || cmd[1] === 0x22);
            ...
            // Valid commands
            expect(validateLaserCommand(Buffer.from([0xC6, 0x01, 0x10, 0x00]))).toBe(true); // Min power
            expect(validateLaserCommand(Buffer.from([0xC6, 0x02, 0x20, 0x00]))).toBe(true); // Max power
            expect(validateLaserCommand(Buffer.from([0xC9, 0x02, 0x00, 0x10, 0x00, 0x00, 0x00]))).toBe(true); // Speed
