...
import { ConnectionHandler, type ConnectionConfig } from './connection-handler.ts';
...
export class TcpServer extends EventEmitter {
    ...
    constructor(config: ServerConfig, connectionConfig: ConnectionConfig, status: Status) {
        super();
        this.config = config;
    ...
    start(): void {
        ...
        this.status.ok(`TCP server listening on ${this.config.hostname}:${this.config.port}`);
        this.emit('started');
    }
    ...
    private handleNewConnection(socket: any): void {
        ...
        if (this.isProcessingConnection || this.currentConnection) {
            ...
            this.status.info(`Connection queued, ${this.connectionQueue.length} in queue`);
        } else {
